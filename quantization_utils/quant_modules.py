...
class QuantAct2(Module):
	...
	def __init__(self,
		...
		self.clip_val = nn.Parameter(torch.Tensor([self.x_max]))
		# self.collect = torch.zeros(100000)
	def __repr__(self):
		return f"{self.__class__.__name__}(activation_bit={self.activation_bit} , full_precision_flag={self.full_precision_flag}, running_stat={self.running_stat}, Act_min: {self.x_min.item()}, Act_max: {self.x_max.item()}),clip_val_grad:{self.clip_val.grad} "
	...
	def initial_clip_val(self):
		self.clip_val.data = self.x_max
		self.repalced = True
	def fix_clip_val(self):
	...
	def forward(self, x):
		...
		"""
		# cnt = torch.histc(x,100000,min=0,max=100)
		# self.collect = torch.add(cnt.cuda(),self.collect.cuda())
		...
		if not self.full_precision_flag  and self.repalced :
			x = torch.where(x<self.clip_val,x,self.clip_val)

			quant_act = self.act_function(x,self.activation_bit,self.x_min,self.clip_val)
			return quant_act

		elif not self.full_precision_flag and not self.repalced:
